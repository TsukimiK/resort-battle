(() => {
  ...
  shell.innerHTML = `<div class="screen-top"><span><i></i> GUNMA / はじまりの小道</span><span id="walk-count">0 STEPS</span></div><div class="map-view"><canvas id="world" width="640" height="416" tabindex="0" aria-label="マップ。矢印キーまたはWASDで移動。草むらを歩くとバトル。気になる場所の前でスペースキー。"></canvas><div class="map-badge">グンマーの小道 <small>☀ はれ</small></div><div id="encounter-flash" hidden><span>！</span></div><div id="npc-dialogue" hidden><div class="npc-dialogue-box"><span class="dialogue-label">SUBARU</span><p id="npc-dialogue-text"></p><small>クリック / SPACE でつづける</small></div></div></div><div class="map-bottom"><div class="map-copy"><span class="dialogue-label">LET'S EXPLORE</span><p id="map-message" role="status" aria-live="polite">草むらには 色違いも まざっているみたい。<br>右の道から 色違いの花園へ行けます。</p><div class="map-legend">矢印キー / WASD：いどう　 SPACE：しらべる</div></div><div class="map-pad" aria-label="移動ボタン"><button data-dir="up" aria-label="上へ移動">▲</button><button data-dir="left" aria-label="左へ移動">◀</button><button id="inspect" aria-label="目の前を調べる">A</button><button data-dir="right" aria-label="右へ移動">▶</button><button data-dir="down" aria-label="下へ移動">▼</button></div></div>`;
  ...
  function travel(destination){
    ...
    message(area==='start'?'グンマーの小道に 戻ってきた！ 右の道は 色違いの花園へ。':'色違いの花園に 到着！ 草むらには 色違いだけが出現。左の道で戻れます。');
  }
  ...
  function isNpc(cx,cy){return area==='start'&&cx===npc.x&&cy===npc.y;}
  ...
  function drawNpc(now){
    if(area!=='start'||!subaruImg.complete)return;
    ctx.imageSmoothingEnabled=true;
    ctx.imageSmoothingQuality='high';
    const bob=Math.sin(now/280)*1.2;
    ctx.drawImage(subaruImg,npc.sx,npc.sy,npc.sw,npc.sh,npc.x*32+npc.dx,npc.y*32+npc.dy+bob,npc.w,npc.h);
  ...
  function inspect(){
    ...
    else message(area==='start'?'右の道は 色違いの花園へ。右下の すばるにも 話しかけてみよう。':'ここは 色違いの花園。草むらで 色違いに会おう！ 左の道で戻れます。');
  }
  ...
  $('reset').onclick=()=>{if(mode==='loading'||mode==='error'||mode==='menu')return;area='start';updateArea();x=6;y=10;steps=0;facing='down';$('walk-count').textContent='0 STEPS';returnToMap();encounterAt=4;message('草むらでは 色違いも出現する。右の道から 色違いの花園へ行ってみよう！');};
  ...
  document.addEventListener('battle-finished',e=>{
    ...
    if(e.detail?.won&&!e.detail?.captured&&e.detail?.enemyId==='subaru'){
