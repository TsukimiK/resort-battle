(() => {
  ...
  shell.innerHTML = `<div class="screen-top"><span><i></i> GUNMA / はじまりの小道</span><span id="walk-count">0 STEPS</span></div><div class="map-view"><canvas id="world" width="640" height="416" tabindex="0" aria-label="マップ。矢印キーまたはWASDで移動。草むらを歩くとバトル。看板の前でスペースキー。"></canvas><div class="map-badge">グンマーの小道 <small>☀ はれ</small></div><div id="encounter-flash" hidden><span>！</span></div></div><div class="map-bottom"><div class="map-copy"><span class="dialogue-label">LET'S EXPLORE</span><p id="map-message" role="status" aria-live="polite">草むらに マタサブロウと ミケケが いるみたい。<br>小道を歩いて 探してみよう！</p><div class="map-legend">矢印キー / WASD：いどう　 SPACE：しらべる</div></div><div class="map-pad" aria-label="移動ボタン"><button data-dir="up" aria-label="上へ移動">▲</button><button data-dir="left" aria-label="左へ移動">◀</button><button id="inspect" aria-label="目の前を調べる">A</button><button data-dir="right" aria-label="右へ移動">▶</button><button data-dir="down" aria-label="下へ移動">▼</button></div></div>`;
  ...
  async function encounter(){mode='transition';held=null;const ticket=++token;$('encounter-flash').hidden=false;const enemyId=Math.random()<0.5?'mikeke':'matasaburo';message('あっ！ 草むらから '+enemies[enemyId].name+'が！');tone(760);await delay(reduced?200:700);if(ticket!==token)return;mode='battle';$('encounter-flash').hidden=true;shell.hidden=true;battle.hidden=false;back.hidden=false;back.textContent='にげて マップへ戻る';reset(enemyId);$('moves').querySelector('button').focus({preventScroll:true});}
  ...
  function inspect(){if(mode!=='map'||moving)return;const [dx,dy]=dirs[facing];if(tile(x+dx,y+dy)==='S'){message('【グンマーの小道】北と南の草むらに マタサブロウと ミケケ。バトルのあとは ひと休み！');tone(620);}else if(tile(x+dx,y+dy)==='W')message('きれいな池だ。水が きらきらしている。');else if(tile(x+dx,y+dy)==='T')message('大きな木が 道をふさいでいる。');else message('草むらを何歩か歩くと マタサブロウや ミケケに出会えるよ。');}
